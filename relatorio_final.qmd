...
top3_final <- top3_final %>%
  group_by(Loja) %>%
  mutate(
    perc_receita = round(Receita_Total / sum(Receita_Total) * 100, 1)
  ) %>%
  ungroup()

top3_final <- top3_final %>%
  mutate(
    legenda = str_c(Produto, " (", perc_receita, "%)")
  )

grafico_top3 <- ggplot(top3_final, aes(
  x = fct_reorder(Produto, Receita_Total, .desc = TRUE),
  y = Receita_Total,
  fill = Loja,
  label = legenda
)) +

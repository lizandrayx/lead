...
banco <- relatorio %>%
  full_join(vendas, by = "SaleID") %>%
  full_join(produtos, by= "ItemID")%>%
  full_join(lojas, by = "StoreID")%>%
  full_join(clientes, by="ClientID")
...
clientela <- clientes %>%
  select(ClientID, Name, Age, Sex, Weight_lbs, Height_dm) %>%
  mutate(
